...
```json
{
  ...
  "artifacts": {
    "memory_used": [
      {
        "id": "4f4f9c4f8c4f4a7c9d3c8e5d4f7a1b2c",
        "created_at": "2026-05-30T10:00:00+00:00",
        "user_message": "What is RoPE?",
        "route": "research",
        "answer": "RoPE is a rotary positional embedding method...",
        "sources": {},
        "artifacts_summary": {}
      }
    ]
...
### 2.6. Memory Agent
Quản lý bộ nhớ hội thoại local. Memory chỉ được áp dụng tự động cho `/api/chat`; các endpoint nghiệp vụ trực tiếp vẫn stateless.

#### 2.6.1. Lấy Memory Gần Đây
- **Endpoint**: `/api/memory`
- **Method**: `GET`
- **Query Params**:
  - `limit` (integer): Số entry cần lấy, mặc định `50`, tối đa `200`.
...
#### 2.6.2. Xóa Toàn Bộ Memory
- **Endpoint**: `/api/memory`
- **Method**: `DELETE`
- **Response Model**: `MemoryClearResponse`
- **Response Example (`200 OK`)**:
```json
{
  "deleted": 12
...
### 2.7. Tìm Kiếm Nghiên Cứu Độc Lập (Direct Research)
...
### 2.8. Mô Tả Ảnh Độc Lập (Direct Vision Describe)
...
### 2.9. Nhận Diện & Đếm Ảnh Độc Lập (Direct Vision Detection)
